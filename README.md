...
当前仓库已实现 M0 至 M37 的可运行纵向切片，并完成后续的引导式 Agent 创建与受控 MCP 迭代。普通创建流程会验证本机 Harness、Provider/模型、当前认证和最小真实调用；Prompt、Skill、Memory、MCP 和项目组件是可选增强，并优先从可搜索的真实目录选择。Pi 的 Native Harness 支持用户明确批准的远程 HTTP 和本地 stdio MCP，external-harness Agent 不再进入 legacy Run 路由。`.agent-stack` 仍是 Component、Stack、Owner、兼容结论、Workflow 和不可变 Version 的唯一便携事实源；SQLite v13 只保留本机引用、创建会话与运行记录。Studio 不自动执行未知项目，密钥原文始终留在 macOS Keychain 边界。
