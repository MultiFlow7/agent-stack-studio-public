...
export class StudioProjectService {
  ...
  async nativeVersionVerified(
    agentId: string,
    agentVersionId: string,
    nativeAgent: Pick<NativeAgentCore, 'list'>,
  ): Promise<boolean> {
    const state = await this.current()
    ...
    const runs = await nativeAgent.list(state.projectPath, 'run')
    return runs.some(
      (run) =>
        run.status === 'succeeded' &&
        run.projectId === state.project!.id &&
        run.projectRevision === version.sourceRevision,
    )
  }
