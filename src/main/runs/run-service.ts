...
export class RunService {
  ...
  start(input: StartRunInput): RunRecord {
    ...
    const route = this.route(input.agentId)
    if (route.kind === 'native') {
      const harnessLabel =
        route.harnessId === 'pi' ? 'Pi' : route.harnessId === 'openclaw' ? 'OpenClaw' : 'Codex CLI'
      throw new AppError(
        'VALIDATION_FAILED',
        `${harnessLabel} Agent 使用 Native Harness 单次运行；请在当前工作台选择“单次运行”。`,
      )
    }
  ...
  route(agentId: string): RunExecutionRoute {
    this.#agents.getActive(agentId)
    const harnessId = this.#agents.nativeHarness(agentId)
    return runExecutionRouteSchema.parse(
      harnessId ? { kind: 'native', harnessId } : { kind: 'legacy' },
    )
  }
