    ...
    createValidatedHandler({
      ...
      handle: () =>
        singleFlight('backup', async () => {
          if (options.selectBackupDestination) {
            const destination = await options.selectBackupDestination()
            return destination
              ? options.maintenance.createBackup(destination)
    ...
    createValidatedHandler({
      ...
      handle: () =>
        singleFlight('select-restore', async () => {
          const sourcePath = options.selectRestoreSource
            ? await options.selectRestoreSource()
            : await showOpenDialog({
                title: '选择 Agent Stack Studio 备份',
                buttonLabel: '检查备份',
                properties: ['openDirectory'],
              }).then((result) => (result.canceled ? null : (result.filePaths[0] ?? null)))
          if (!sourcePath) return { status: 'cancelled' as const }
