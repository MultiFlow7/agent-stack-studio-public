...
export class AgentService {
  ...
  #nativeAgent: { cancelProject(projectId: string): number } | null = null
  ...
  connectNativeAgent(nativeAgent: { cancelProject(projectId: string): number }): void {
  ...
  update(input: UpdateAgentInput): AgentDetail | Promise<AgentDetail> {
    ...
    const currentMode =
      state?.project?.stack.executionMode ??
      this.#repository.getDetail(input.id).agent.executionMode
    if (input.executionMode !== currentMode && input.executionMode !== 'external-harness') {
      throw new AppError(
        'VALIDATION_FAILED',
        '旧 Agent Loop、Workflow 和 Hybrid 模式不再接受新切换；只能保留历史值或显式迁移到 Native Harness。',
      )
    }
  ...
  async delete(agentId: string): Promise<DeleteAgentResult> {
    const projectId = this.#projects?.activeComposition(agentId)?.project?.id
    if (projectId) this.#nativeAgent?.cancelProject(projectId)
  ...
  nativeHarness(agentId: string): HarnessId | null {
    const project = this.#projects?.activeComposition(agentId)?.project
    if (!project || project.stack.executionMode !== 'external-harness') return null
    const controllerOwner = project.stack.capabilityOwners.find(
      ({ capability }) => capability === 'execution-controller',
    )
    if (!controllerOwner || !project.stack.componentIds.includes(controllerOwner.componentId)) {
      return null
    }
    const controller = project.components.find(({ id }) => id === controllerOwner.componentId)
    return harnessIdFromAdapter(controller?.descriptor.runtimeAdapter ?? null)
  }
