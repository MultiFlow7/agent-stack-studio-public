...
import { createCliModelAuth, type CliModelAuthHandle } from './model-auth-host'
...
export async function createCliPublishing(
  ...
  let modelAuth: CliModelAuthHandle | null = null
  try {
    ...
    modelAuth = await createCliModelAuth(rootPath, core, dataRoot)
    projects.connectModelAuth(modelAuth.controller)
    ...
    return {
      ...
      service: new PublishService({
        ...
        nativeVerification: (agentId, agentVersionId) =>
          projects.nativeVersionVerified(agentId, agentVersionId),
      }),
      close() {
        modelAuth?.close()
