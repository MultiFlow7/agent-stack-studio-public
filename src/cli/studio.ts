...
Agent：agent create|inspect|configure|validate|freeze|versions|version|export|secret
...
async function createCliPublishing(
  ...
  return host.createCliPublishing(rootPath, core, nativeAgent, dataRootOverride)
}
...
function routeProductCommand(parsed: ParsedArguments): ProductCommandRoute {
  ...
  if (group === 'agent') {
    ...
    if (action === 'freeze') {
      return {
        parsed: withPositionals(parsed, ['stack', 'freeze', ...rest]),
...
async function executeCliCommandInternal(
  ...
  if (group === 'publish') {
    const owned = dependencies.publishing
      ? null
      : await createCliPublishing(rootPath, core, nativeAgent, flag(parsed, 'data-dir'))
  ...
  if (group === 'chat' && (action === 'send' || !action)) {
    ...
    const requestId = requestValue
      ...
      : undefined
    const result = await nativeAgent.execute(
      {
        projectPath: rootPath,
        kind: 'chat',
