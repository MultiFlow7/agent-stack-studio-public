...
import { ConnectedModelAuthenticationSection } from './ModelAuthenticationSection'
...
const validationIssueLabels: Record<string, string> = {
  ...
  OWNER_REQUIRED: '需要选择负责实现',
  OWNER_INVALID: '负责实现无效',
      ...
      >
        <header>
          ...
          <div>
            ...
            <p>
              {validation?.status === 'ready'
                ? 'Stack 与兼容性前置条件已通过；仍需完成 Harness 与模型验证。'
                : hasHardBlock
                  ? '先处理下列错误，再重新检查。'
