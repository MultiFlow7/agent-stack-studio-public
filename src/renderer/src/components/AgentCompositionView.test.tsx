...
describe('AgentCompositionView compatibility actions', () => {
  it('runs static inspection and makes a cancelled source relink visible', async () => {
    ...
    window.studio = {
      ...
      modelAuth: {
        status: vi.fn().mockResolvedValue({
          ...
          selection: null,
          readiness: {
            state: 'harness-not-selected',
            ready: false,
            stackCompatible: false,
            harnessExecutable: false,
            configuration: null,
            authentication: null,
            verification: {
              state: 'not-run',
            ...
            blockers: [
              {
                code: 'harness-not-selected',
                message: '尚未选择 Harness。',
                recoveryAction: '选择 Harness。',
              },
            ],
          },
        }),
    ...
    const modelAuthHeading = await screen.findByRole('heading', { name: '模型与认证' })
    expect(
      screen.getByText('Stack editor').compareDocumentPosition(modelAuthHeading) &
