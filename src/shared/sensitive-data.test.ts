...
describe('sensitive diagnostic redaction', () => {
  ...
  it('removes Electron invoke details before an error reaches product UI', () => {
    expect(
      sanitizedIpcErrorMessage(
        new Error(
          "Error invoking remote method 'runs:start': Error: 请在 Native Harness 工作台运行。",
        ),
        '无法完成操作。',
      ),
    ).toBe('请在 Native Harness 工作台运行。')
  })
