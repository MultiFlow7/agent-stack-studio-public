  ...
  async restoreSnapshot(
    ...
    try {
      const [current, snapshot] = await Promise.all([
        this.#readLocked(filePath, true),
        parseFile(path.resolve(snapshotPath)),
      ])
      if (current.project.revision !== expectedRevision) {
        throw new StudioCoreError(
          'REVISION_CONFLICT',
          '项目在安装失败后又被修改，为避免覆盖新工作已停止自动恢复。',
          {
            details: { expectedRevision, actualRevision: current.project.revision, snapshotPath },
          },
        )
      }
      if (
        snapshot.project.id !== current.project.id ||
        snapshot.project.revision >= current.project.revision
      ) {
        throw new StudioCoreError('PROJECT_INVALID', '安装快照不属于当前项目或不是更早修订。', {
          details: { snapshotPath },
        })
      }
      await atomicWrite(current.path, snapshot.project, true)
      return {
        path: current.path,
        project: snapshot.project,
        migrated: false,
        recovered: true,
        integrity: verifyProjectIntegrity(snapshot.project),
      }
    } finally {
      await release()
    }
