...
describe('StudioCore local Coding Agent workflow', () => {
  it('stores only portable model requirements, snapshots them, and invalidates them on Harness change', async () => {
    const root = await createRoot()
    const core = new StudioCore()
    let state = await core.initProject(root, { name: 'Portable model configuration' })
    state = await core.selectKnownHarness(root, 'pi', {
      expectedRevision: state.project.revision,
    })
    state = await core.updateModelConfiguration(
      root,
      {
    ...
    expect(state.project.modelConfiguration).toEqual({
      providerId: 'anthropic',
      modelId: 'claude-sonnet-4-20250514',
      credentialRequirement: { method: 'api-key', credentialKind: 'api-key' },
    })
    expect(JSON.stringify(state.project.modelConfiguration)).not.toMatch(
      /binding|keychain|configured|verified|secret/i,
    )
    const frozen = await core.freezeVersion(root, { expectedRevision: state.project.revision })
    expect(frozen.version.snapshot.modelConfiguration).toEqual(state.project.modelConfiguration)

    const changed = await core.selectKnownHarness(root, 'codex', {
      expectedRevision: frozen.result.project.revision,
    })
    expect(changed.project.modelConfiguration).toBeNull()
  })
