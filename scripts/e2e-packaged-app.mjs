...
export async function runPackagedAppE2e(options = {}) {
  ...
  try {
    ...
    await waitForExpression(client, "document.body.innerText.includes('Agent 就绪')")
    ...
    await waitForExpression(client, "document.body.innerText.includes('Agent 已阻断')")
    ...
    await waitForExpression(
      client,
      "document.body.innerText.includes('准备状态') && document.body.innerText.includes('就绪') && document.body.innerText.includes('1 个组件') && document.body.innerText.includes('最近 Run') && document.body.innerText.includes('已完成') && document.body.innerText.includes('尚未发布')",
    )
    ...
    await waitForExpression(
      client,
      "Boolean([...document.querySelectorAll('.agent-row')].find((element) => element.textContent?.includes('Packaged CLI E2E') && element.textContent?.includes('版本 1') && element.textContent?.includes('准备状态：就绪') && element.textContent?.includes('最近 Run：已完成') && element.textContent?.includes('发布：未发布')))",
    )
    await evaluate(
      client,
      `(() => {
