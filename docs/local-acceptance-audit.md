...
当前审计结果为 120 个 production 源文件、7 个可达目的地、6 个输入提示和 2 组最终包验收控制，未处置项为 0。`fixture` 只允许位于测试树或最终 packaged E2E 输入；Main 中的验收控制只改变临时数据根、截图/导出目标或启动检查，不替换 Studio Core、SQLite、项目协议、Runtime 或 CLI。
...
- 最终 `.app` 通过 Chromium Accessibility Tree 检查主导航与 `main` landmarks、7 个一级入口、全局搜索和创建操作；当前为 21 个可见按钮、0 个无名称按钮。
- packaged E2E 逐个聚焦并打开 7 个一级入口，验证对应 `h1` 与 `aria-current=page`，输出 `NAVIGATION_REACHABILITY VERIFIED (7)` 和 `PACKAGED_ACCESSIBILITY_TREE VERIFIED`。
