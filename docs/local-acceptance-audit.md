...
- 最终 `.app` 通过 Chromium Accessibility Tree 检查主导航与 `main` landmarks、5 个一级入口、顶栏当前项目、全局搜索和创建操作；所有可见按钮必须有名称。
- packaged E2E 逐个聚焦并打开 5 个一级入口，验证对应 `h1` 与 `aria-current=page`，输出 `NAVIGATION_REACHABILITY VERIFIED (5)` 和 `PACKAGED_ACCESSIBILITY_TREE VERIFIED`。
