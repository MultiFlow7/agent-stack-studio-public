...
| RR-029 | 用户验证 5 | 每个里程碑全套 format/lint/type/test/build/CLI/package/verify/E2E | COMPLETE | M13–M27 本地通过；M27 为 79 files / 262 tests、build、CLI、arm64 DMG/ZIP、包验证、导航/AX packaged E2E 和完整无凭证 dry-run；报告 5 verified、3 skipped、2 disabled、0 blocked；公开 Intel CI run `32444148841` 同样通过 | 无 | 持续对每个里程碑执行同一验证 |
| RR-030 | 用户实施要求 | 每个里程碑提交、推送并确认 GitHub CI | COMPLETE | M13–M27 实现 CI 均成功；M27 私有实现 `a52437c` 与公开实现 `ebf6bfd` 共享 tree `191c580716fe4901bd26798e4483ed7f8e29f940`，公开 run `32444148841` 成功 | 公开 GitHub hosted runner | 每切片保留相同 tree 证据并等待 CI |
...
| RR-037 | 用户公开仓库要求 | 公开代码和 CI 不泄露私有历史、凭证或个人信息 | COMPLETE | 私有已共享历史不改写且不镜像；公开仓库仅含 noreply 作者提交；M27 `verify:public-snapshot` 检查 264 个文件；私有/公开实现 tree 一致 | 无凭证入库 | 3 个 verifier tests；M27 公开 run `32444148841` 成功 |
