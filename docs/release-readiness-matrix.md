...
| RR-004 | 分发文档 8；ADR 0007/0019 | Intel x64 在 GitHub macOS Intel runner 执行同一包检查 | PARTIAL | 官方标准标签 `macos-15-intel` 已配置；当前 HEAD `ff6dd50` 的 run `32624691417` / job `97158079034` 为 `runner_id: 0`、0 steps；check-run annotation 明确说明账户近期付款失败或支出上限不足，job 未启动 | GitHub 账户 Billing & plans | 账户付款/支出上限恢复后从当前 HEAD 重跑；不更换为 arm64 代替 x64 |
...
| RR-028 | 分发文档 8；ADR 0019 | Intel 真机人工安装与核心流程走查 | EXTERNAL-BLOCKED | x64 CI 契约和官方 runner 标签已配置；当前 job 因账户 Billing 阻断而未获分配，且尚无独立 Intel 真机人工证据 | Intel 真机/人工环境；GitHub 账户 Billing | 外部条件到位后执行 |
| RR-029 | 用户验证 5 | 每个里程碑全套 format/lint/type/test/build/CLI/package/verify/E2E | COMPLETE | M13–M31 历史通过；M37 当前为 99 files / 344 tests、build、arm64 App/DMG/ZIP、包验证、无 Node CLI、Doctor、旧模型迁移与跨重启备份恢复 packaged E2E | 无 | 最终外部凭证闭环后重跑完整 dry-run |
| RR-030 | 用户实施要求 | 每个里程碑提交、推送并确认 GitHub CI | PARTIAL | M13–M31 历史 CI 成功；M32–M37 提交均已推送，但当前 run `32624691417` 的 annotation 明确为账户近期付款失败或支出上限不足，未执行任何 checkout/测试 step | GitHub 账户 Billing & plans | 保留 run/job/annotation 证据；付款或支出上限恢复后从当前 HEAD 重跑 |
