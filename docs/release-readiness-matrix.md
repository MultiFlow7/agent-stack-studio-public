...
| RR-031 | 用户停止条件 14 | 最终报告关联需求 ID、提交、测试、包路径、截图、CI | COMPLETE | `generate:final-report` 从两张矩阵与证据 manifest 生成 `release/final-local-completeness-report.json`，M30 含 150 条需求、9 条命令、4 个包路径和 23 张本地截图；推送后注入私有/公开提交与 CI | 无 | `verify:final-report` strict + artifacts 验证 |
...
| RR-037 | 用户公开仓库要求 | 公开代码和 CI 不泄露私有历史、凭证或个人信息 | COMPLETE | 私有已共享历史不改写且不镜像；公开仓库只推送 noreply 作者的隐私审核快照；M30 `verify:public-snapshot` 检查 280 个文件并拒绝未审核二进制；23 张截图全部 Git ignored | 无凭证入库 | 4 个 verifier tests；私有/公开 tree 一致性由最终报告验证 |
