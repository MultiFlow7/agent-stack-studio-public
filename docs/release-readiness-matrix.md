...
| RR-029 | 用户验证 5 | 每个里程碑全套 format/lint/type/test/build/CLI/package/verify/E2E | COMPLETE | M13–M31 本地通过；M31 为 86 files / 295 tests、build、CLI、arm64 DMG/ZIP、包验证、兼容处置/归档恢复 packaged E2E 和完整无凭证 dry-run；报告 5 verified、3 skipped、2 disabled、0 blocked | 无 | 持续对每个里程碑执行同一验证 |
| RR-030 | 用户实施要求 | 每个里程碑提交、推送并确认 GitHub CI | COMPLETE | M13–M31 实现 CI 均成功；M31 私有实现 `bbb6c79` 与公开快照 `bb21883` 共享 tree `93a030958c7aac06544433301a948a88b87d49c7`，公开 run `32559072071` 成功 | 公开 GitHub hosted runner | 每切片保留相同 tree 证据并等待 CI |
