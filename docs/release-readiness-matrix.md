...
| RR-037 | 用户公开仓库要求 | 公开代码和 CI 不泄露私有历史、凭证或个人信息 | COMPLETE | 私有已共享历史不改写且不镜像；公开仓库只推送 noreply 作者、以公开 main 为父的隐私审核快照；当前 `verify:public-snapshot` 检查 348 个文件，公开 `f08fe1c` 与私有 `41feac2` tree `c29532e` 一致且 CI 成功；23 张截图及两份外部验收 JSON 全部 Git ignored | 无凭证、Runtime/工作区/本机身份入库 | 4 个 verifier tests；commit parent/author/tree 与公开 CI 复核 |
