...
| RR-029 | 用户验证 5 | 每个里程碑全套 format/lint/type/test/build/CLI/package/verify/E2E | COMPLETE | M13–M26 本地通过；M26 为 78 files / 259 tests、build、CLI、arm64 DMG/ZIP、包验证、工作区命令中心 packaged E2E 和完整无凭证 dry-run；报告 5 verified、3 skipped、2 disabled、0 blocked；Intel CI 待实现提交同步后落证 | 无 | 持续对每个里程碑执行同一验证 |
...
| RR-037 | 用户公开仓库要求 | 公开代码和 CI 不泄露私有历史、凭证或个人信息 | COMPLETE | 私有已共享历史不改写且不镜像；公开仓库仅含 noreply 作者的审计快照；M26 `verify:public-snapshot` 检查 260 个文件 | 无凭证入库 | 3 个 verifier tests；公开 snapshot `f2b6e62`；run `32438973147`；M26 继续以相同 tree 门禁同步 |
