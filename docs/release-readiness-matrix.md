...
| RR-004 | 分发文档 8；ADR 0007/0019 | Intel x64 在 GitHub macOS Intel runner 执行同一包检查 | COMPLETE | 公开隐私审核快照 `de52cd9` 的免费 macOS CI run `32682598762` / job `97301739150` 在官方 `macos-15-intel` runner 完成 checkout、锁定依赖、全量项目检查和 Intel 应用打包/检查，用时 8m28s | 无付费 GitHub 依赖；独立 Intel 真机人工安装仍见 RR-028 | 保持公开 noreply 快照与隐私扫描；不把自动 x64 包证据表述成真机人工安装 |
...
| RR-022 | 用户分发要求 6；ADR 0019 | 备份、恢复、回滚与跨机器 Keychain 缺失边界稳定 | COMPLETE | maintenance + secret/IPC tests；最终 arm64 `.app` 真实创建备份、改写 Artifact、显式确认、退出/重启并还原；Recovery 保留恢复前值 | 无 | `PACKAGED_BACKUP_RESTORE VERIFIED`；`packaged-app-e2e-backup-restore.png` |
...
| RR-028 | 分发文档 8；ADR 0019 | 独立无 Node/npm/开发仓库 Mac 的 DMG 拖装与 GUI+CLI+Multica 人工终验，并覆盖 Intel 真机 | EXTERNAL-BLOCKED | 本机最终 arm64 `.app` 黑盒闭环、包内 Node/CLI、备份恢复、受限 PATH Doctor 均已通过；x64 CI 自动包门禁已由公开 run `32682598762` 通过；尚无独立 Mac，尤其是 Intel 真机的安装闭环证据 | 独立 macOS 机器、Intel 真机与人工环境 | 外部条件到位后按 runbook 执行；不需要 GitHub 付费计划，不修改业务协议 |
| RR-029 | 用户验证 5 | 每个里程碑全套 format/lint/type/test/build/CLI/package/verify/E2E | COMPLETE | M13–M31 历史通过；M32–M37 当前为 102 files / 362 tests、build、arm64 App/DMG/ZIP、包验证、packaged Pi/OpenClaw Codex simulation、GUI/CLI 真实 Multica、无 Node CLI、Doctor、旧模型迁移与跨重启备份恢复证据；公开快照 `de52cd9` 的 Intel CI run `32682598762` 同 tree 通过 | 无 | Apple 凭证与独立安装环境闭环后再跑有凭证 release dry-run |
| RR-030 | 用户实施要求 | 每个里程碑提交、推送并确认 GitHub CI | COMPLETE | M13–M31 历史 CI 成功；M32–M37 私有功能提交 `173c25e` 已推送；同 tree 以无私有历史、noreply 作者的公开快照 `de52cd9` 推送，免费 macOS CI run `32682598762` 完整成功 | 无付费 GitHub 依赖 | 后续切片继续使用隐私审核公开快照，不公开私有历史 |
...
| RR-037 | 用户公开仓库要求 | 公开代码和 CI 不泄露私有历史、凭证或个人信息 | COMPLETE | 私有已共享历史不改写且不镜像；公开仓库只推送 noreply 作者、以既有公开分支为父的隐私审核快照；`verify:public-snapshot` 检查 349 个文件，公开 `de52cd9` 与私有 `173c25e` tree `b6a490b` 一致且 CI 成功；本地截图与外部验收 JSON 全部 Git ignored | 无凭证、Runtime/工作区/本机身份入库 | 4 个 verifier tests；commit parent/author/tree 与公开 CI 复核 |
...
| RR-039 | M27/M32；用户停止条件 3/7 | 正式分发前持续拒绝占位、断路导航、无名称控件和 mock-only 业务路径 | COMPLETE | 验收 manifest 只分类 UI 提示与黑盒包控制；自动扫描 production 源码；最终 `.app` 遍历 M32 收敛后 5 个一级入口并检查 AX tree | 无 | verifier tests；`LOCAL_ACCEPTANCE_AUDIT VERIFIED`；`NAVIGATION_REACHABILITY VERIFIED (5)`；`PACKAGED_ACCESSIBILITY_TREE VERIFIED` |
| RR-040 | M30/M32；ADR 0009/0011 | Agent-first 主流程与项目设置次级化在正式包中完整可达 | COMPLETE | Agent/组件/运行/发布/设置 5 个一级入口；Agent 组装、冻结、Native run/chat/publish 入口；顶栏打开项目设置 | 无 | packaged E2E、AX tree 与中文截图 |
