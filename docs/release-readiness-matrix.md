...
| RR-029 | 用户验证 5 | 每个里程碑全套 format/lint/type/test/build/CLI/package/verify/E2E | COMPLETE | M13–M28 本地通过；M28 为 80 files / 266 tests、build、CLI、arm64 DMG/ZIP、包验证、全入口/AX packaged E2E 和完整无凭证 dry-run；报告 5 verified、3 skipped、2 disabled、0 blocked；公开 Intel CI run `32446386099` 同样通过 | 无 | 持续对每个里程碑执行同一验证 |
| RR-030 | 用户实施要求 | 每个里程碑提交、推送并确认 GitHub CI | COMPLETE | M13–M28 实现 CI 均成功；M28 私有实现 `2c9fdd9` 与公开实现 `7748351` 共享 tree `e3846498ce6d4621491f39ac1988e28f2a7cff96`，公开 run `32446386099` 成功 | 公开 GitHub hosted runner | 每切片保留相同 tree 证据并等待 CI |
| RR-031 | 用户停止条件 14 | 最终报告关联需求 ID、提交、测试、包路径、截图、CI | COMPLETE | `generate:final-report` 从两张矩阵与证据 manifest 生成 `release/final-local-completeness-report.json`，含 136 条需求、9 条命令、4 个包路径、23 张本地截图及私有/公开提交与 CI | 无 | `verify:final-report` strict + artifacts 验证；M28 公开 run `32446386099` |
...
| RR-037 | 用户公开仓库要求 | 公开代码和 CI 不泄露私有历史、凭证或个人信息 | COMPLETE | 私有已共享历史不改写且不镜像；公开仓库仅含 noreply 作者提交；M28 `verify:public-snapshot` 检查 269 个文件并拒绝未审核二进制；23 张截图全部 Git ignored；私有/公开实现 tree 一致 | 无凭证入库 | 4 个 verifier tests；M28 公开 run `32446386099` 成功 |
| RR-038 | M26；用户分发要求 7 | 工作区摘要、搜索和实体直达无需为正式分发改写项目、数据库、Runtime 或 CLI | COMPLETE | 只读服务组合既有 Project/Agent/Component/Run/Experiment 事实；搜索只返回页面/UUID/白名单操作；`.agent-stack` v2、包 v2、SQLite v8、Runtime 与 CLI 项目协议不变 | 无 | Core/Service/严格 IPC/Preload/Renderer tests；最终 arm64 `.app` 输出 `WORKSPACE_COMMAND_CENTER VERIFIED` |
| RR-039 | M27；用户停止条件 3/7 | 正式分发前持续拒绝占位、断路导航、无名称控件和 mock-only 业务路径 | COMPLETE | 验收 manifest 只分类 UI 提示与黑盒包控制；自动扫描 production 源码；最终 `.app` 遍历 7 个入口并检查 AX tree | 无 | verifier tests；`LOCAL_ACCEPTANCE_AUDIT VERIFIED`；`NAVIGATION_REACHABILITY VERIFIED`；`PACKAGED_ACCESSIBILITY_TREE VERIFIED` |
