...
| LC-027 | UI 14；ADR 0003 | 更正 Descriptor 但不伪造兼容性证据 | COMPLETE | 组件库结构化更正；信任/许可/Owner 决策单独确认 | `component update`/descriptor API 保留 evidence level | `studio-core.test.ts`、`compatibility-assessment.test.ts` | 最终包 Agent 兼容性截图显示证据来源 | M30 修正；Descriptor 编辑不再自动变为 user-confirmed |
| LC-028 | UI 14；ADR 0003/0010 | Component 加入/移出 Stack、active/archived/all 筛选、归档/恢复与受保护永久删除均可操作 | COMPLETE | Agent 组装器加入/移出/Owner；组件库归档筛选、恢复、确认/取消、引用保护 | Core/CLI/IPC/Preload 共享 `.agent-stack`；`component list --scope`、`restore`、`archive`、`delete` | `ComponentCatalogView.test.tsx`、`studio-core.test.ts`、`cli-contract.test.ts`、IPC tests | packaged 归档筛选→恢复→Agent 立即可选截图 | M31 补齐恢复闭环；Stack/Workflow/Version 历史引用继续阻止删除 |
...
| LC-030 | PRD 6；技术架构 8 | Workflow 是版本化 DAG，保存时检测直接/间接循环引用 | COMPLETE | Agent 组装器结构化创建/节点/边/删除/冻结与只读 DAG 图示；保留空、取消、失败、冲突和外部刷新状态 | `.agent-stack` v2 + 共享 Studio Core；`studio workflow create/list/inspect/node-add/node-remove/edge-add/edge-remove/freeze`；6 个严格 IPC | `workflow-core.test.ts` 覆盖直接/间接循环、幂等冻结、历史不变与 Component 引用保护；ProjectStore v0/v1/v2 兼容；CLI/IPC/UI tests | `VERSIONED_WORKFLOW_DAG VERIFIED`；DAG/循环失败截图；GUI↔CLI 与 v2 包一致 | M30 收敛到 Agent；项目 Workflow 不自动获得 Runtime 信任 |
...
| LC-061 | ADR 0003；项目格式 | `.agent-stack` 有版本化 JSON Schema、revision 和唯一事实来源 | COMPLETE | Agent 页呈现 Stack/Owner/Workflow/Version；项目设置显示 v2/revision/完整性 | Core/CLI 共用 `PROJECT_FORMAT_VERSION=2`；v1 Schema 保留 | Core/store/CLI/release compatibility/legacy migration tests | 最终包同时携带 project v1/v2 Schema | M30 确认六类便携事实不再读写 SQLite 副本 |
...
| LC-063 | ADR 0003；UI 14 | 外部 CLI/编辑器修改触发 GUI 刷新并提示新 revision | COMPLETE | 顶栏当前项目与 Agent 组装器刷新提示 | CLI 修改 | service/UI revision conflict/external modification tests | packaged GUI↔CLI fixture 与外部刷新截图 | M30 持续回归 |
...
| LC-071 | 用户实施要求 / 迁移失败恢复 | 每个数据库迁移中途失败都回滚，旧 schema/数据可继续读取或安全重试 | COMPLETE | 启动统一迁移错误边界 | `migrate` 每版本独立事务；legacy portable migration 先备份项目再单事务写引用 | `migrations.test.ts`、`legacy-portable-migration.test.ts` 覆盖新装、v1→v9、幂等、失败恢复、冲突与较新版拒绝 | packaged 启动全新 v9 并验证引用 | M30 扩展到 SQLite 与项目跨存储迁移 |
...
| LC-083 | AGENTS.md；UI 10/11 | 主要流程覆盖加载、空、成功、失败、取消、冲突、外部刷新和完整键盘路径 | COMPLETE | 8 条跨领域旅程逐项分类八状态；可发生状态有 UI 行为证据，只读/单写入边界有显式原因 | 结构化 Core/Service/CLI/IPC 错误与 revision 冲突 | `final-evidence.json`；`evidence-ledger.test.mjs`；每个状态参照至少一项自动化 | 23 张本地 packaged 中文截图覆盖空/成功/失败/取消/冲突/外部刷新 | M28 关闭；新旅程必须补齐八状态分类 |
| LC-084 | PRODUCT Accessibility；UI 10 | WCAG 2.2 AA：语义、焦点、对比、错误、图标名称、减少动态 | COMPLETE | skip link、landmarks、可见 3px 焦点、文字+图标状态、Dialog 焦点约束；输入提示不代替 label | N/A | 对比度计算覆盖正文/按钮/错误/成功/警告；reduced-motion/sr-only contract；Dialog/App 键盘 tests | Chromium Accessibility Tree：main/navigation、21 个可见按钮、0 无名称；逐页键盘可达 | M27 关闭；未来新增控件继续受 AX/源码门禁 |
...
| LC-086 | UI 2；PRD 4 | 顶栏提供当前项目、全局搜索、运行状态和应用级操作 | COMPLETE | 真实项目/revision/验证状态、切换/项目设置、Run 状态、`⌘K` 搜索、创建与实体直达 | 只读 `CommandCenterService`；严格 snapshot/search IPC 与目的地白名单 | Core/Service/IPC/App/CommandPalette 及 Component/Run/Experiment 直达测试；Project summary 不吞外部修改回归 | `WORKSPACE_COMMAND_CENTER VERIFIED`；当前项目与命令中心截图 | M30 将项目切换升为全局上下文，不增加执行权限 |
...
| LC-088 | 技术架构 6；用户目标 | 导出不含密钥的可移植 Agent Stack Package | COMPLETE | 项目设置提供原生保存、成功/取消/失败和键盘路径 | 共享 Core `exportProjectPackage`；`studio project export --output` | package builder/hash/path audit、Workflow v2 保留、Core/CLI/GUI 一致、严格 IPC、release compatibility 测试 | 最终 `.app` GUI 与包内 CLI 读取同一 Component/Workflow Version；`SECRET_FREE_PORTABLE_PACKAGE VERIFIED` | M30 保留为次级项目设置，非 Agent 组装副本 |
...
| LC-092 | README；用户验证 5 | GUI 中发现包内 CLI 的准确路径且不修改 PATH/Shell profile | COMPLETE | 项目设置 | CLI path Main API | StudioProjectView/service tests | `verify-macos-package` 检查路径 | M30 收纳为次级工具 |
...
| LC-095 | 用户验证 5 | 中文主要流程及空、失败、冲突状态有截图复核 | COMPLETE | 全部是最终 `.app` 中文界面 | N/A | `final-evidence.json` 验证 23 个截图路径、状态、旅程与 producer token | 设置、来源空/失败、命令中心、Run 超时、Experiment 部分取消、Agent/组件冲突、项目外部刷新和 Workflow 循环全部有本地截图 | M28 关闭；截图固定 Git ignored，不发布个人路径 |
| LC-096 | 用户验证 5 | 扫描 TODO/FIXME/占位按钮/断路导航/mock-only 并逐项分类 | COMPLETE | 6 个一级导航均启用、渲染且命令中心可检索；项目设置为顶栏次级入口 | N/A | `verify:local-acceptance` 与 verifier tests；未处置项 0 | 最终 `.app` 逐页打开并确认 `aria-current`；packaged harness 不替换 Core | M30 导航收敛；门禁进入 `npm run check` |
| LC-097 | 用户停止条件 | 每个冻结需求至少一个自动化证据，用户流程另有 GUI/packaged 证据 | COMPLETE | 机器报告保留每行 GUI 证据；8 条旅程单独索引 | 机器报告保留每行 CLI/API 证据 | `verify:evidence-ledger` 覆盖 97 LC + 39 RR，COMPLETE LC 拒绝缺失自动化证据 | 23 个本地 packaged 截图 producer 与所有用户流程证据逐项绑定 | M28 关闭；最终 strict 报告继续拒绝任何未完成状态 |
| LC-098 | M30；产品主入口 | Agent 页承载 Stack、Owner、兼容性、Workflow 和 Version 冻结 | COMPLETE | `AgentCompositionView` 连续编辑区；冻结后直达 Run/Experiment/Publish | Agent/Project 严格 IPC 和 Studio Core | App、AgentCompositionView、StackEditor、WorkflowEditor tests | 最终 `.app` Agent-first 主流程与中文截图 | M30 完成 |
| LC-099 | M30；ADR 0003/0009 | `.agent-stack` 是六类便携事实的唯一来源 | COMPLETE | Agent/组件库/项目设置是同一项目投影 | CLI 与 GUI 共用 Core；SQLite 只存稳定引用 | StudioProjectService/ComponentService/AgentRepository tests | packaged GUI↔CLI 交叉读写同一 fixture | 无双库同步层 |
| LC-100 | M30；组件流程 | 导入组件后 Agent 立即可选 | COMPLETE | 组件库导入成功即刷新 Agent 组装器 | `installDeclaredComponents` 单次 revision-aware 项目写入 | `studio-project-service.test.ts` immediately selectable；ComponentCatalogView tests | packaged 导入→Agent 选择→Stack 截图 | 不写 SQLite Component 副本 |
| LC-101 | M30/M31；兼容性 | 系统给出六状态可解释 Compatibility Assessment，Unknown 显示具体缺失机器证据 | COMPLETE | 中文状态/说明、证据、阻断、方法、时间；每个 action 映射按钮/表单/外部步骤 | 共享评估器返回严格结构化 assessments/actions | `compatibility-assessment.test.ts`、Core/IPC/Renderer tests | Pi/MRAgent 机器证据不足、策略、Adapter 与失败/取消/成功截图 | 不显示原始枚举，不要求用户猜或编辑 JSON |
| LC-102 | M30/M31；ADR 0010 证据安全 | Descriptor/策略编辑不能提升或伪造技术证据；契约变更使旧证据失效 | COMPLETE | 完整结构表单，验证等级只读，Receipt/Artifact/superseded/审计可见，Cancel 零写入 | Core 忽略提交的 validation/evidence；只有静态/契约/Runtime 系统路径写证据；revision 保护 | `studio-core.test.ts`、`ComponentCatalogView.test.tsx`、schema/compatibility tests | 策略选择不升级、冲突和证据失效截图 | 旧 user-confirmed 只映射为人工决策，保留历史 |
| LC-103 | M30；SQLite v9 | 本机 Agent 以稳定 ID 引用项目与不可变 Version | COMPLETE | Agent 列表/详情保留本机身份与项目上下文 | `agent_project_links` + `project-reference` snapshot；发布前内存实体化 | migrations/AgentRepository/Run/Experiment/Publish tests | packaged 冻结 Agent Version 并完成 Run/Experiment/Publish 预检 | 历史引用保护不变 |
| LC-104 | M30；历史迁移 | 旧 SQLite Agent/Component 无损、幂等迁移，失败可恢复 | COMPLETE | 启动失败保留原数据与 migration backup，不静默降级 | 先项目后 SQLite 事务；冲突/混合引用/无法归属安全拒绝 | `legacy-portable-migration.test.ts`、`migrations.test.ts` | packaged 新装 v9；迁移错误保留恢复指引 | 较新 schema/项目格式拒绝降级 |
| LC-105 | M30；一致性 | GUI→CLI、CLI→GUI、revision 冲突与外部修改一致 | COMPLETE | 顶栏当前项目和 Agent 编辑器显示新 revision/冲突恢复 | CLI `--revision` 与 ProjectStore 排他写边界 | ProjectStore/StudioProjectService/Renderer tests | `PACKAGED_GUI_CLI_BIDIRECTIONAL VERIFIED` 与冲突/外部刷新截图 | 不使用同步数据库 |
| LC-106 | M30/M31；ADR 0010 安全运行验证 | 未知代码不执行；契约测试与受信运行验证分级，覆盖超时、取消、异常恢复、脱敏、Artifact 和 Receipt | COMPLETE | 未知组件只显示静态诊断；运行验证有进度/取消/重试 | Core 前置契约测试 + 精确 Adapter 白名单 + 全新 Cordis 子进程内置 Adapter 真实启停 | Pi/MRAgent、`trusted-compatibility-runtime.test.ts`、Core/IPC/sensitive-data tests | packaged 验证失败/取消/成功与脱敏 Artifact/Receipt | stdout/stderr 不跨边界；Cordis 不越过 Runtime 边界 |
| LC-107 | M30；信息架构 | 一级导航收敛为 Agent/组件库/发现/实验/运行/设置，项目设置次级化 | COMPLETE | 六个一级入口、顶栏项目切换、完整键盘路径和 WCAG 2.2 AA | 导航/目的地白名单与严格偏好设置 | App/local-acceptance/accessibility verifier tests | `NAVIGATION_REACHABILITY VERIFIED (6)` 与 AX tree/中文页面截图 | M30 完成 |

## 首次冻结审计结论

当前冻结共 107 项。M30 在原有闭环上完成 Agent-first 导航、项目唯一事实源、导入后立即可选、可解释兼容性、证据等级修正、SQLite v9 稳定引用和旧数据可恢复迁移；当前为 COMPLETE 102、EXTERNAL-BLOCKED 1、EXPLICITLY-OUT-OF-SCOPE 4，MISSING/PARTIAL/UNTESTED 均为 0。真实 Multica Transport 单独保持外部阻断；文档明确排除的云端与团队能力不进入本地实现。
