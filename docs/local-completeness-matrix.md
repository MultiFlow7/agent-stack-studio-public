...
| LC-028 | UI 14；ADR 0003 | Component 加入/移出 Stack、归档与永久删除均可操作 | COMPLETE | Agent 组装器加入/移出/Owner；组件库归档、确认/取消、引用保护 | 包内 CLI 对每一步读取同一 `.agent-stack`；Core 全部命令 | `AgentCompositionView.test.tsx`、`ComponentCatalogView.test.tsx`、Core/CLI 引用保护 | `PROJECT_COMPONENT_LIFECYCLE VERIFIED`；Agent 组装与历史阻断截图 | M30 收敛入口；取消零写入，项目/Workflow 历史引用继续阻止删除 |
...
| LC-101 | M30；兼容性 | 系统给出六状态可解释 Compatibility Assessment | COMPLETE | 文字显示状态、证据、阻断、方法、时间和下一步 | 共享评估器返回结构化 assessments | `compatibility-assessment.test.ts`、studio-core/runtime compiler tests | 兼容通过、需配置、需 Adapter 和阻断截图 | 不要求用户猜 Descriptor |
| LC-102 | M30；证据安全 | Descriptor 编辑不自动提升为 user-confirmed | COMPLETE | 信任/许可/Owner 人工决策与技术兼容性分开 | `updateComponentDescriptor` 保留 evidence level | studio-core evidence regression tests | 最终 Agent 详情显示实际证据来源 | M30 修正已确认缺陷 |
...
| LC-106 | M30；安全运行验证 | 未知代码不执行；受信验证有超时、取消、脱敏、Artifact 和 Receipt | COMPLETE | 未知组件只显示静态诊断与可执行修复路径 | Runtime 精确 Adapter 白名单 + 独立子进程 | runtime/compiler/controller/sensitive-data/receipt tests | packaged 受信 Run 超时/取消/异常恢复与脱敏 Artifact/Receipt | Cordis 不越过 Runtime 边界 |
