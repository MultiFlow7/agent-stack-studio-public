...
```bash
studio agent create|inspect|configure|model|validate|freeze|versions|version|export|secret
...
- `agent model status`：输出 GUI/冻结共享的 Harness、Provider/模型、认证和最小调用事实；不读取凭证原文，不发起模型调用。
- `agent model configure --provider <id> --model <id> --auth-method api-key|official-login|existing-login --revision <n>`：更新可移植要求。`--model` 可使用推荐项，也可在当前 Harness capability 允许时使用符合安全格式的完整模型 ID。API Key 方式必须额外使用 `--stdin`，原文不得出现在 argv 或 JSON。
- `agent model verify --confirm-cost`：用当前 binding 发起一次最小模型调用；缺少显式费用确认时拒绝执行。
- `secret set <account> --stdin`：只从标准输入读取密钥；`--service` 默认 `studio.agentstack.desktop`，可用 `--label` 设置钥匙串显示名称。这是高级未绑定入口，不会使 Agent 变为就绪。
