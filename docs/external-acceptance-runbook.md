...
printf '%s' "$PI_PROVIDER_API_KEY" | studio agent model configure --project /path/to/pi-acceptance --provider openai --model gpt-5.1 --auth-method api-key --revision <revision> --stdin --json
studio agent model status --project /path/to/pi-acceptance --json
studio agent model verify --project /path/to/pi-acceptance --confirm-cost --json
...
1. 冻结前的 model status 必须同时证明 Stack/compatibility、Harness executable、凭证有效与同一配置哈希的最小模型调用；单纯 probe 或 Codex simulation 不得计入。
2. `data.status` 为 `succeeded`，`data.harness` 是目标 Harness，`harnessVersion` 满足固定范围。
3. Run 为非交互调用；相同 idempotency key 重试返回同一本机结果，不产生第二条执行事实。
4. 两次 Chat 使用同一个 `sessionId`，第二次回复证明真实会话延续。
5. `projectHash` 是当前项目事实的 SHA-256；聊天/回复只保存在 `.agent-stack-local`，不进入冻结 Version。
6. 至少一个项目包含 Prompt、Markdown Memory、Skill；MCP 仅在 Harness 支持且用户已批准的情况下验收，降级必须逐项显示。

保存证据时只保留状态、Harness/version、Provider/模型、配置哈希、session 是否复用、usage 是否可用、project hash 是否记录与模型层类型；不保存原始 session/request/binding ID、Keychain locator、Prompt、响应、Provider 凭证或 stderr。另用随机 opaque canary 扫描项目、SQLite、history、日志、备份、导出、发布 payload 和 CLI stdout/stderr，唯一允许命中位置是受控 Keychain adapter。
...
Intel GitHub CI 必须从待发布精确 tree 运行同一 `check`、package、verify、packaged E2E 和 release dry-run。按用户决策不购买 GitHub 付费计划：私有历史不公开，当前 tree 以 noreply 隐私审核快照推送到公开仓库并使用免费 Actions。公开快照 `de52cd9` 的 run `32682598762` / Intel job `97301739150` 已完成项目检查、Intel x64 应用打包/检查与 packaged E2E，用时 8m28s。历史私有仓库 0-step job 不作为代码证据。
